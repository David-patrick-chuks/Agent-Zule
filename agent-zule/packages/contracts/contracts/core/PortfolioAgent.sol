...
contract PortfolioAgent is IPortfolioAgent, AccessControl, ReentrancyGuard, Pausable {
    ...
    modifier notEmergencyStopped() {
        require(!emergencyStop, "PortfolioAgent: emergency stop active");
        _;
    }
        ...
        validPositions(params.newPositions) 
    {
        ...
        // Check permissions
        (bool hasPermission, bool requiresVoting) = permissionManager.checkPermission(
            user, 
            keccak256("REBALANCE"), 
            _calculateTotalValue(params.newPositions)
        );
        
        require(hasPermission, "PortfolioAgent: insufficient permissions");
     ...
     */
    function hasPermission(address user, bytes32 action) external view override returns (bool hasPermission) {
        (hasPermission,) = permissionManager.checkPermission(user, action, 0);
    }
    ...
    function _escalateToVoting(address user, bytes32 actionType, bytes memory data) internal {
        string memory description = string(abi.encodePacked("Escalated action: ", actionType));
        votingEngine.createVote(description, actionType, data, 86400); // 24 hours
    }
}
