...
contract ExecutionEngine is IExecutionEngine, AccessControl, ReentrancyGuard, Pausable {
    ...
    modifier notEmergencyPaused() {
        require(!emergencyPause, "ExecutionEngine: emergency pause active");
        _;
    }
     ...
     */
    function getRegisteredStrategies() external view override returns (StrategyConfig[] memory strategies) {
        strategies = new StrategyConfig[](registeredStrategies.length);
        
        for (uint256 i = 0; i < registeredStrategies.length; i++) {
            strategies[i] = strategies[registeredStrategies[i]];
        }
    }
    ...
    ) external returns (bool success, bytes memory returnData, uint256 gasUsed, uint256 actualSlippage) {
